...
const DAILY_VALUES = {
  ...
  carbs: 300, // g
  fiber: 25, // g
  totalSugars: 50, // g (no official DV, using WHO recommendation)
  addedSugars: 50, // g
  protein: 50, // g
  vitaminD: 20, // mcg
  calcium: 1300, // mg
...
function calculateDailyValue(nutrient: keyof typeof DAILY_VALUES, amount: number): number {
  const dv = DAILY_VALUES[nutrient]
  return Math.round((amount / dv) * 100)
}

export function NutritionFactsPanel({ nutrition, servings, className }: NutritionFactsPanelProps) {
  return (
    <Card className={className}>
      ...
      <CardContent className="space-y-1 text-sm">
        ...
        <div className="flex justify-between border-b border-gray-300 pb-1">
          <span className="font-bold">Total Fat {nutrition.fat}g</span>
        ...
        <div className="flex justify-between pl-4">
          ...
          <span className="font-bold">{calculateDailyValue("saturatedFat", nutrition.saturatedFat)}%</span>
        </div>

        {/* Trans Fat */}
        <div className="pl-4 border-b border-gray-300 pb-1">
          <span>Trans Fat {nutrition.transFat}g</span>
        </div>

        {/* Cholesterol */}
        <div className="flex justify-between border-b border-gray-300 pb-1">
          <span className="font-bold">Cholesterol {nutrition.cholesterol}mg</span>
          <span className="font-bold">{calculateDailyValue("cholesterol", nutrition.cholesterol)}%</span>
        </div>
        ...
        {/* Total Sugars */}
        <div className="pl-4">
          <span>Total Sugars {nutrition.totalSugars}g</span>
        </div>

        {/* Added Sugars */}
        <div className="flex justify-between pl-8 border-b border-gray-300 pb-1">
          <span>Added Sugars {nutrition.addedSugars}g</span>
          <span className="font-bold">{calculateDailyValue("addedSugars", nutrition.addedSugars)}%</span>
        </div>
        ...
        {nutrition.allergens.length > 0 && (
          <div className="pt-2 border-t">
            ...
            <span className="text-xs">{nutrition.allergens.join(", ")}</span>
          </div>
        )}
      </CardContent>
    </Card>
